...
let resolvedDbPath: string | undefined | null = null;

/**
 * The real session DB path: ask opencode itself (it honors XDG_DATA_HOME and
 * any internal relocation, and prints the exact location), falling back to
 * the conventional literal when the binary is unavailable. Memoized.
 */
function defaultDbPath(): string {
  if (resolvedDbPath !== null) return resolvedDbPath;
  try {
    const proc = Bun.spawnSync(["opencode", "db", "path"], { stdout: "pipe", stderr: "ignore" });
    const out = proc.stdout.toString().trim();
    resolvedDbPath = proc.exitCode === 0 && out ? out : DEFAULT_DB;
  } catch {
...
export function salvageSession(parentId: string, options: { dbPath?: string } = {}): SalvageReport {
  ...
  try {
    ...
    for (const child of children) {
      ...
      // Extraction is bounded to the same window the report uses — a huge
      // child output should not cost extra regex/parse passes.
      const bounded = textTruncated ? text.slice(0, MAX_TEXT_CHARS) : text;
      const findings = extractJsonFindings(bounded);
